...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def get_user_info(self, request, pk=None):
        ...
        result = UserInfoSerializer(user_info).data
        if result["anonymous_mode"] and result["display_name"]!=my_display_name:
            return Response({"error_code":"UserNotExist"},
                                status=status.HTTP_404_NOT_FOUND)
        return Response(result, status=status.HTTP_200_OK)
    ...
    def set_user_info(self, request, pk=None):
        ...
        if UserInfo.objects.filter(user=user).exists():
            is_same_name=setting_items["display_name"]==UserInfo.objects.get(user=user).display_name
        ...
        if not serializer.is_valid():
            ...
            try:
                ...
                pass
            try:
                if errors["display_name"][0].code=="unique"and not is_same_name:
...
class PostViewSet(viewsets.ModelViewSet):
    ...
    def get_post(self, request, pk=None):
        ...
        try:
            ...
            sum_record = Post.objects.filter(keywords__keyword__exact=keyword).count()
            start = min(start, sum_record)
            end = min(end, sum_record)
            posts = Post.objects.filter(keywords__keyword__exact=keyword).order_by("-created")[start:end]
        except KeyError:
    ...
    @action(methods=["get"], 
            permission_classes=[IsAuthenticated],
            detail=False,url_path="get_keyword_post", url_name="get_keyword_post")
