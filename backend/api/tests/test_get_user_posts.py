...
class GetUserPostsTest(APITestCase):
    def setUp(self):
        ...
        self.url = reverse("api:post-get_user_post",
                           kwargs={"pk": "public_user"})
        self.post_sum = len(Post.objects.all().filter(
            post_sender=self.user_info))
    ...
    def test_error_get_not_exist_user_info(self):
        url = reverse("api:post-get_user_post",
